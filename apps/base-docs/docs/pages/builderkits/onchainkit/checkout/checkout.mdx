...
```tsx
// Example code - this won't be executed during build
...
:::info
The `Checkout` component must be used within `WagmiProvider`, `QueryClientProvider`, and `OnchainKitProvider`. See the [Getting Started](../getting-started#add-providers) guide for full setup instructions.
:::
...
If you'd like to create product metadata programmatically or implement a multi-product checkout, please see [Advanced Usage](#advanced-usage).
...
:::note
The examples below are wrapped in our `App` component which provides the necessary WagmiProvider, QueryClientProvider, and OnchainKitProvider context.
:::
...
This callback will return a [LifecycleStatusData](#advanced-usage) object including the [TransactionReceipt](https://github.com/wevm/viem/blob/main/src/types/transaction.ts#L38) and `chargeId`.
...
You can use our Checkout [`LifecycleStatus`](types#lifecyclestatus) and the `onStatus` prop to listen to transaction states.
...
- [`LifecycleStatus`](types#lifecyclestatus)
- [`CheckoutReact`](types#checkoutreact)
- [`CheckoutButtonReact`](types#checkoutbuttonreact)
- [`CheckoutStatusReact`](types#checkoutstatusreact)
