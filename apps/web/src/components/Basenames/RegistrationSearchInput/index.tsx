...
export default function RegistrationSearchInput({
  ...
  const mutedButtonClasses = classNames(
    'flex w-full flex-row items-center text-gray-60 justify-between transition-colors hover:bg-[#F9F9F9] active:bg-[#EAEAEB] text-ellipsis',
    {
  ...
  const handleRedirect = useCallback(() => {
    logEvent(
      'link_click',
      {
        ...
        context: 'claimed_name_redirect',
      },
      AnalyticsEventImportance.low,
    );
    router.push(`/name/${formatBaseEthDomain(debouncedSearch, basenameChain.id)}`);
  }, [router, debouncedSearch, basenameChain.id]);
    ...
    >
      ...
      <div className={dropdownClasses}>
        ...
        {invalidWithMessage ? (
          ...
          <>
            <button className={mutedButtonClasses} type="button" onClick={handleRedirect}>
              <span className="truncate">
                {formatBaseEthDomain(debouncedSearch, basenameChain.id)} is already registered. Go
                to profile
              </span>
              <Icon name="diagonalUpArrow" width={14} height={14} color="black" />
            </button>
