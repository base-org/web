...
import { USERNAME_DESCRIPTION_MAX_LENGTH, UsernameTextRecords } from 'apps/web/src/utils/usernames';
...
export default function UsernameDescriptionField({ labelChildren }: UsernameDescriptionFieldProps) {
  ...
  const { data: baseEnsDescription, isLoading: baseEnsDescriptionIsLoading } = useEnsText({
    ...
    key: UsernameTextRecords.Description,
    chainId: baseSepolia.id,
