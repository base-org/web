...
Show attestation on ENV name with [`Badge`](badge) component.

Use [`OnchainKitProvider`](../config/onchainkit-provider) or [`Identity`](identity) component with the `schemaId` prop.
