...
// This will create a spend permission of 0.0001 ETHfor the sub account during sub account creation and store it in the context.
const createSubAccount = useCallback(async () => {
  ...
  const spendPermissionData = walletConnectResponse?.accounts[0].capabilities?.spendPermission;
