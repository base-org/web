...
export function useRemoveFollowsCallback(name: string): UseRemoveFollowsCallbackReturnValue {
  ...
  const removeFollows = useCallback(
    async (follows: string[]) => {
      ...
      logEventWithContext('remove_follows_transaction_initiated', ActionType.click);
      ...
      try {
        ...
        logError(e, 'Remove Follow transaction canceled');
        logEventWithContext('remove_follows_transaction_canceled', ActionType.change);
      }
    },
    [
      address,
