...
export function useIsFinalizationPeriodElapsed(timestamp: number | undefined) {
  ...
  if (FINALIZATION_PERIOD_SECONDS && timestamp) {
    return {
      hasElapsed:
        Math.floor(Date.now() / 1000) > timestamp + FINALIZATION_PERIOD_SECONDS.toNumber(),
      challengeWindowEndTime: timestamp + FINALIZATION_PERIOD_SECONDS.toNumber(),
    };
  }
  return {
    hasElapsed: false,
