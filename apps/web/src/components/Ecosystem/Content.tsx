		...
		return [
			...new Set(
				selectedSubcategories.map(
					(subcategory) =>
						Object.keys(config).find((category) =>
							config[category].includes(subcategory),
						) ?? "all",
				),
			),
		];
	...
	const filteredEcosystemApps = useMemo(() => {
		return decoratedEcosystemApps.filter((app) => {
			const isSubcategoryMatched =
				...
				selectedSubcategories.includes(app.subcategory);
			const isSearched =
				search === "" || app.searchName.includes(search.toLowerCase());
